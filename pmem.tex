...
\begin{abstract}
  %%
  We evaluated Intel\textsuperscript{\textregistered} Optane\textsuperscript{\tiny TM} DC Persistent Memory and found surprising behavior not discussed in prior work.  In particular, we found that the Linux persistent memory (PMEM) support automatically maps persistent memory in large pages.  The effect of large pages for PMEM and small pages for DRAM is enormous, dwarfing other effects discussed in the literature.  Using the \textit{Polybench} high-performance computing benchmark suite and controlling for mapped page size, we evaluate PMEM performance relative to DRAM.  We find that Intel's persistent memory is highly sensitive to data locality, size, and access patterns, which becomes clearer by optimizing both virtual memory page size and optimizing locality for data layout.  We found PMEM performance comparable to DRAM performance for a number of tests, and significant performance improvements when optimized for data locality.
  %%  
\end{abstract}
...
%%
We found PMEM is more sensitive to memory locality than DRAM; this is due to differences in the hardware implementing these memory technologies.  The decision to default PMEM to large pages provides a substantial boost in page locality.  As datasets grow in size, using large pages yields better TLB locality because more actual PMEM can be described with a fixed number of TLB entries.  PMEM's support for interleaving is most beneficial for data with both good locality and high bandwidth requirements; non-interleaved PMEM works comparably well to iPMEM with good data locality and moderate bandwidth requirements, such as with CPUs employing larger caches.
%%

%%
We caution those evaluating PMEM to ensure they are properly controlling for page size; it's impact is so large that not controlling for it can yield unexpected results.  Similarly, we observe that \textit{optimizing} data locality for workloads can lead to substantial performance gains.  Such gains are clearly sensitive to the behavior of the underlying memory.  Thus, failing to properly optimize for data locality will impact performance.
%%
...
%%
We thank the anonymous reviewers for their valuable feedback. This research was partially funded under NSF award 1822972, US Dept. of Energy UNITY, Intel Corporation, and Exascale Computing Project SICM (Simplified Interface to Complex Memory).
%%
...
% Omit Appendix for final version.
% input{appendix}
